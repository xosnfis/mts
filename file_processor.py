...
def _df_to_markdown(df) -> str:
    """Convert DataFrame to Markdown table, capping at 200 rows."""
    import pandas as pd
...
def extract_text_from_any(
    filename: str,
    data: bytes,
) -> Optional[str]:
    """
    Parse a document by file extension and return extracted text.
    Returns None if the format is unsupported or parsing fails.
    ...
    Returns:
        Extracted text string (possibly truncated), or None on failure.
    """
...
def build_file_context_block(filename: str, text: str) -> str:
    """Wrap extracted text in a [FILE CONTEXT] block for system prompt injection."""
    return (
